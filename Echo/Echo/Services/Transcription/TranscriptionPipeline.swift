...
enum TranscriptionProgress: Equatable, Sendable {
    case preparing
    case uploading
    case processing
    case filtering
...
protocol TranscriptionPipelineProtocol: Sendable {
...
@MainActor
final class DefaultTranscriptionPipeline: TranscriptionPipelineProtocol {
    ...
    private func runPipeline(
        ...
        EchoLog.audio.debug(
            "Pipeline done: provider=\(providerId.rawValue, privacy: .public) filtered=\(wasFiltered, privacy: .public) chars=\(finalText.count, privacy: .public) elapsed=\(elapsedStr, privacy: .public)s"
        )
