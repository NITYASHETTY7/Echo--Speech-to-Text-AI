...
public typealias AVAudioRecorderFactory = (URL, [String: Any]) throws -> AVAudioRecorderProtocol
...
public final class AudioRecorder: NSObject {
    ...
    public private(set) var currentPowerDB: Float = -160
...
extension AudioRecorder: AudioSessionDelegate {
    ...
    public nonisolated func audioSessionWasInterrupted(shouldResume: Bool) {
        Task { @MainActor in
    ...
    public nonisolated func audioRouteDidChange(reason: AVAudioSession.RouteChangeReason) {
        Task { @MainActor in
    ...
    public nonisolated func audioSessionWasReset() {
        Task { @MainActor in
